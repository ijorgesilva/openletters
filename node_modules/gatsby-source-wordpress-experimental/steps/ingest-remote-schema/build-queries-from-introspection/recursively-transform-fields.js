...
function transformField({
  ...
  if (fieldType.kind === `SCALAR` || fieldType.kind === `NON_NULL` && ofType.kind === `SCALAR` || fieldType.kind === `LIST` && fieldType.ofType.kind === `SCALAR` || // a list of enums has no type name, so findTypeName above finds the enum type
  ...
  if (isAMediaItemNode && hasIdField) {
    ...
    var _fieldType$possibleTy;

    const isAnInterfaceTypeOfGatsbyNodes = // if this is an interface
    typeKind === `INTERFACE` && ( // and every possible type is a future gatsby node
    fieldType === null || fieldType === void 0 ? void 0 : (_fieldType$possibleTy = fieldType.possibleTypes) === null || _fieldType$possibleTy === void 0 ? void 0 : _fieldType$possibleTy.every(possibleType => gatsbyNodesInfo.typeNames.includes(possibleType.name)));
    return {
      fieldName: fieldName,
      fields: isAnInterfaceTypeOfGatsbyNodes ? // we need the typename for interfaces
